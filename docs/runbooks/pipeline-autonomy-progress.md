...
Le contrôle 34714281896 est réussi : 103/103 pages récupérées, chacune avec une ligne d’audience datée explicite et passée depuis plus de 24 heures, aucun report annoncé, deux mentions « Vente non requise ». Preuves durables : `docs/audits/licitor-absent-detail-evidence-20260912.json` (date, ligne source, empreinte et horodatage par URL). Les 103 fiches avaient une date catalogue nulle et au plus une observation ; aucune fenêtre de vente concurrente. Mise à jour conditionnelle sur identité, date nulle, résultat nul et absence de modification depuis le contrôle : 101 statuts past, 2 withdrawn. Aucune déduction de vente conclue ; aucune suppression directe. La rétention existante conserve son contrôle des preuves statistiques. Vérifier ensuite le bridge et la mesure de fraîcheur ; poursuivre l’intégration du contrôle périodique des fiches absentes.

## Vérification des archives et du reliquat Vench

Après le passage automatique de la rétention, les 103 fiches Licitor ont quitté le catalogue : 103 bridges détachés, 103 résultats unknown, 103 exclus de l’entraînement statistique. Vérification SQL effective, pas simple décompte des suppressions.

Le contrôle des six dernières fiches Vench anciennes (run GitHub 34714668452) réussit : six audiences explicites du 1er au 9 septembre 2026, dates précédemment nulles en base, aucun report dans les données récupérées. Preuve `docs/audits/vench-stale-detail-evidence-20260912.json`. Mise à jour conditionnelle des six statuts past et dates, sans déduire un résultat ; aucune suppression directe. Vérifier leur archivage au prochain tick de rétention.

Le parseur Licitor reconnaît désormais les mentions propres à une ligne « Vente non requise », « Vente reportée » et « Vente annulée ». Trois tests ajoutés prouvent le retrait sans résultat inventé, la protection du report contre expiration et le maintien d’un résultat inconnu ; 19 tests Licitor et 5 tests d’audit réussis, Ruff/diff check réussis. PR 128 ouverte ; finaliser la description sur ce périmètre et attendre la CI de la dernière révision avant intégration. Le run Vench 34713609273 est toujours actif, sur l’ancien code PR126 ; ne pas le relancer sur un simple délai d’observation.

La relecture des 13 anciennes fausses adjudications identifie une concaténation supplémentaire : une description terminée par « prix d’adjudication » pouvait emprunter le montant d’un autre bloc (Houilles 445000 EUR, en réalité mise à prix). Révision `72e31ef` de la PR128 : chaque bloc est analysé séparément, test de régression ajouté. Recomposition en lecture seule des 13 raw_payload : 13 statuts upcoming, 13 résultats nuls. Fichiers locaux `/private/tmp/immojudis-future-results-review.json` et `/private/tmp/immojudis-future-results-recomputed.json`. Les quarantaines de production restent en place tant que les autres réserves de périmètre n’ont pas été examinées et le correctif déployé. Suite complète : 1227 tests Python réussis, 32 ignorés sans PostgreSQL ; CI 72e31ef en cours. Ne pas intégrer une ancienne révision sur la foi de tests précédents.

PR128 intégrée après tous les contrôles réussis : `144a8ec23bbfa21456c26774719c08dff7c7916b`, 19:46:54 UTC. Vérifier le déploiement puis réexaminer les 13 quarantaines. Les six archives Vench sont effectivement détachées avec résultat unknown et entraînement exclu, comme les 103 Licitor.

Branche suivante `feat/pipeline-enrichment-unknown` : la file Python et la fonction SQL de claim excluaient le statut unknown. Correction de l’admissibilité, sans modifier les protections d’expiration/quarantaine. Migration `20260912194309_enrich_unknown_active_listings.sql` restaure uniquement les dernières tâches annulées par l’ancien filtre, en conservant les budgets de tentatives et les autres annulations. Tests PostgreSQL 9/9 et fiabilité 12/12 passent. Aucun job associé à une fiche actuellement unknown en production au contrôle ; cette protection couvre les prochaines publications factuelles incomplètes.
