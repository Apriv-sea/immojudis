...
export function allowedTarget(value: string, method: string, body: string): boolean {
  try {
    ...
    if (method === "GET")
      return (
        !body &&
        (u.hostname === "cessions.immobilier-etat.gouv.fr" ||
          u.pathname === "/robots.txt" ||
          u.pathname.startsWith("/encheres-immobilieres/"))
      );
    if (
      method !== "POST" ||
      u.hostname !== "www.petitesaffiches.fr" ||
      !u.pathname.startsWith("/encheres-immobilieres/")
    )
      return false;
  ...
  try {
    ...
    try {
      const response = await fetch(url, {
        ...
        ...(new URL(url).hostname === "cessions.immobilier-etat.gouv.fr"
