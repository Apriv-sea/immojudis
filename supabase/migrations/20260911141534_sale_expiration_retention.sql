...
create or replace function app_private.invoke_sale_retention_endpoint()
returns bigint
language plpgsql
security definer
set search_path = ''
...
begin
  ...
  select net.http_get(
    url => pg_catalog.rtrim(endpoint_url, '/') || '/api/cron/sale-retention',
...
revoke all on function app_private.invoke_sale_retention_endpoint()
from public, anon, authenticated, service_role;


select cron.schedule('immojudis-sale-retention', '*/5 * * * *', 'select app_private.invoke_sale_retention_endpoint();');
