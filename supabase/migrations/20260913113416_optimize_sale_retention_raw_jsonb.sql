...
-- Normalize object-shaped raw JSON once.  PostgreSQL may detoast a large
-- jsonb value on each operator access; keeping this local copy preserves the
-- existing retention rules while avoiding repeated decompression in the hot
-- queue predicates.
...
declare
  ...
  end_at timestamptz;
  v_raw jsonb := case
    when jsonb_typeof(p_raw) = 'object' then p_raw || '{}'::jsonb
    else p_raw
  end;
  raw_date text := coalesce(v_raw->>'sale_date', '');
...
begin
  if lower(coalesce(v_raw->>'status','')) ~ '(postponed|reported|report[eé]e?)' then return null; end if;
  if jsonb_path_exists(coalesce(v_raw,'{}'), '$.source_conflicts[*] ? (@.field == "sale_date")') then return null; end if;
  if lower(coalesce(p_status,'')) in ('postponed','reported','reportee','reporté','reportée') then return null; end if;
  foreach schedule in array array[p_procedure->'sale_window', p_procedure->'sale_session', v_raw->'source_sale_schedule'] loop
